...
//                finish()
//            } else {
            if (Utils.getPref(getActivity(), Constant.IS_FIRST_TIME, false)) {
                val changePage = Intent(getActivity(), MainActivity::class.java)
                startNewActivity(changePage)
                finish()
            } else {
                val i = Intent(getActivity(), IntroActivity::class.java)
                startNewActivity(i)
                finish()
            }
        }
                ...
                .onPositive(MaterialDialog.SingleButtonCallback { materialDialog, dialogAction ->
                    try {
                        ...
                        try {
                            ...
                            startNewActivity(Intent(Intent.ACTION_VIEW, Uri.parse("http://play.google.com/store/apps/details?id=$appPackageName")))
                        }
                        finish()
                    } catch (e: Exception) {
                        e.printStackTrace()
